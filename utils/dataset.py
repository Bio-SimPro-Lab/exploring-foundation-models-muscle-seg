...
class Public_dataset(Dataset):
    def __init__(self,args, img_folder, mask_folder, img_list,phase='train',sample_num=50,channel_num=1,normalize_type='sam',crop=False,crop_size=1024,targets=['bg','vl','vm','vi','rf','sar','gr','am','sm','st','bfl','bfs','al'],part_list=['all'],cls=-1,if_prompt=True,prompt_type='point',region_type='largest_3',label_mapping=None,if_spatial=True,delete_empty_masks=True):
        '''
    ...
    def setup_transformations(self):
        if self.phase =='train':
            ...
            transformations = [transforms.RandomResizedCrop(self.crop_size, scale=(0.8, 1.2), interpolation=InterpolationMode.NEAREST),
                     transforms.RandomAffine(10, translate=(0.1, 0.1),shear=5, interpolation=InterpolationMode.NEAREST)]
        else:
            transformations = []
        ...
        if self.normalize_type == 'sam':
            ...
            self.normalize = transforms.Compose([transforms.Lambda(lambda x: (x - torch.min(x)) / (torch.max(x) - torch.min(x)))])

        self.transform_spatial = transforms.Compose(transformations)
    ...
    def __getitem__(self, index):
        ...
        if mask_path.startswith('/'):
            mask_path = mask_path[1:]
        full_out_path = os.path.join(self.img_folder, img_path.strip())
        out_path = full_out_path.replace('_IP', '_OP')
        out_path = out_path.replace('_0000_', '_0001_')

        img = Image.open(os.path.join(self.img_folder, img_path.strip())).convert('L')
        img_out = Image.open(os.path.join(out_path)).convert('L')
        msk = Image.open(os.path.join(self.mask_folder, mask_path.strip())).convert('L')

        img = transforms.ToTensor()(img)
        img_out = transforms.ToTensor()(img_out)
        # Combine images into a single tensor with two channels
        combined_img = torch.cat((img, img_out), dim=0)

        # Calculate the mean of the first two channels to create the third channel
        mean_channel = torch.mean(combined_img[:2, :, :], dim=0, keepdim=True)
        combined_img = torch.cat((combined_img, mean_channel), dim=0)

        combined_img_np = combined_img.numpy()
        combined_img_np = (combined_img_np * 255).astype(np.uint8)

        # Create an RGB image from the combined channels
        combined_img_np = np.transpose(combined_img_np, (1, 2, 0))  # Convert to HWC format
        combined_pil_img = Image.fromarray(combined_img_np, mode='RGB')

        img = transforms.Resize((self.args.image_size,self.args.image_size))(combined_pil_img)
    ...
    def apply_transformations(self, img, msk):
        ...
        if self.phase=='train' :
            ...
            msk = torch.tensor(mask_cls).long()
           # msk = torch.nn.functional.one_hot(msk, num_classes=13).permute(2, 0, 1).float()
        if self.phase =='val':
            img =self.normalize(img)
