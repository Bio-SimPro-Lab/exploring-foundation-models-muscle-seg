# -*- coding: utf-8 -*-
"""
Created on Thu May 11 11:21:06 2023
...
def NLL(true_OHE, confidence, epsilon=1e-5):
    assert true_OHE.shape == confidence.shape
    confidence = np.clip(confidence, epsilon, 1.0)
    log_softmax_probs = np.log(confidence)
    elementwise_mult = np.sum(true_OHE * log_softmax_probs, axis=1)
    return -np.mean(elementwise_mult)
...
def compute_calibration(true_labels, pred_labels, confidences, num_bins=10):
    ...
    for b in range(num_bins):
        ...
        if len(selected) > 0:
            if b<num_bins/2:
                bin_accuracies[b] = 1-np.mean(true_labels[selected] == pred_labels[selected])
            else:
...
def _reliability_diagram_subplot(ax, bin_data, 
    ...
    if draw_ece:
        ...
        ax.text(0.98, 0.02, "ECE=%.4f" % ece, color="black", 
